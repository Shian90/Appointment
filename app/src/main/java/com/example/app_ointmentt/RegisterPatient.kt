...
class RegisterPatient : AppCompatActivity() {
    ...
    private fun registerPatient(){
        ...
        if(usernamePatient.isEmpty() || emailPatientReg.isEmpty() || passwordPatientReg.isEmpty() || dobPatient.isEmpty() || contactPatient.isEmpty() || bloodGroupPatient.isEmpty()||genderPatient.isEmpty()|| addressDistrictPatient.isEmpty()|| addressAreaPatient.isEmpty()){
            Toast.makeText(this, "Please fill everything up", LENGTH_SHORT).show()
        }else{

            FirebaseAuth.getInstance().createUserWithEmailAndPassword(emailPatientReg, passwordPatientReg)
                .addOnSuccessListener {
                    ...
                    val patientAttribute = PatientClass(usernamePatient, emailPatientReg, passwordPatientReg, dobPatient, contactPatient, bloodGroupPatient, genderPatient, addressDistrictPatient, addressAreaPatient)

                    refPat.document("$pid").set(patientAttribute)
                        .addOnSuccessListener {
                            Toast.makeText(this, "User is added to database", LENGTH_SHORT).show()
                            val loggedInPatient = FirebaseAuth.getInstance().currentUser
                            loggedInPatient?.sendEmailVerification()
                                ?.addOnSuccessListener {
                ...
                .addOnFailureListener {
                    Toast.makeText(this, "User not created. Exception: ${it.message}", LENGTH_SHORT).show()
                }
        }
    }
}
